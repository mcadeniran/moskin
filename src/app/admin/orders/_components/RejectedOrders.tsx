...
const fetchRejectedOrders = (): Promise<Order[]> => fetch('/api/admin/orders/status/REJECTED', {method: 'GET'}).then(res => res.json());

const RejectedOrders = () => {
  const router = useRouter();
  const {isLoading, data: orders, error} = useQuery({queryKey: ['rejectedOrders'], queryFn: fetchRejectedOrders});

  if (isLoading) return <p className="">Loading Rejected Orders</p>;
  ...
  return (
    <div className='mt-4 min- h-[calc(100vh-23.8rem)]'>
      {
        orders?.length === 0 ?
          <div>No order has been rejected</div>
          :
          <Table>
            <TableHeader>
              <TableRow>
