...
const fetchCancelledOrders = (): Promise<Order[]> => fetch('/api/admin/orders/status/CANCELLED', {method: 'GET'}).then(res => res.json());


const CancelledOrders = () => {
  const router = useRouter();
  const {isLoading, data: orders, error} = useQuery({queryKey: ['cancelledOrders'], queryFn: fetchCancelledOrders});

  if (isLoading) return <p className="">Loading Cancelled Orders</p>;
  ...
  return (
    <div className='mt-4 min- h-[calc(100vh-23.8rem)]'>
      {
        orders?.length === 0 ?
          <div>No cancelled order found</div>
          :
          <Table>
            <TableHeader>
              <TableRow>
